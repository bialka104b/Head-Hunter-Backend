...
export class AuthRecord implements UserLoginRequest {
	...
	static async findTokenId(
		...
		const [result] = (await pool.execute(
			"SELECT `currentTokenId` FROM `users` WHERE `currentTokenId` = :currentTokenId",
			{
				currentTokenId,
			},
	...
	protected async generateToken(userId: string): Promise<string> {
		let token;
		let userWithThisToken: any = null;
		do {
			token = uuid();
			userWithThisToken = await AuthRecord.findTokenId(token);
		} while (!!userWithThisToken);
		await pool.execute(
			"UPDATE `users` SET `currentTokenId` = :token WHERE `id` = :id ",
			{
				token,
				id: userId,
			},
	...
	static async logout(id: string) {
		await pool.execute(
			"UPDATE `users` SET `currentTokenId` = NULL WHERE `id` = :id ",
			{
				id,
			},
		);
	...
	async login(): Promise<UserLoginResultsResponse> {
		const [result] = (await pool.execute(
			"SELECT `id`,`email`, `password`, `currentTokenId`, `role` FROM `users` WHERE `email` = :email AND `isActive` = :isActive",
			{
				email: this.email,
				isActive: true,
			},
		...
		if (!result[0]) {
			return {
				isLogin: false,
				message:
					"Not User Found. Possible that your status is non active.",
			};
		}
